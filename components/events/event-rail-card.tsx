  ...
  return (
    <Link href={href} className="group flex min-w-[300px] gap-3 rounded-xl border border-border bg-card p-3 shadow-sm ui-hover-lift ui-press focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2" aria-label={`Open event ${title}`}>
      <div className="relative h-20 w-28 shrink-0 overflow-hidden rounded-md bg-muted">
        {imageUrl ? <Image src={imageUrl} alt={title} fill sizes="112px" className="object-cover ui-trans motion-safe:group-hover:scale-[1.02] motion-safe:group-focus-visible:scale-[1.02]" /> : <div className="flex h-full items-center justify-center text-xs text-muted-foreground">No image</div>}
      </div>
