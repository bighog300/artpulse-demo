...
function createMemoryDb() {
  ...
  const memoryDb = {
    ...
    notification: {
      upsert: async (args: { where: { dedupeKey: string }; create: Omit<NotificationRow, "id" | "createdAt" | "status" | "href"> & { href?: string | null } }) => {
        const key = args.where.dedupeKey;
      ...
      updateMany: async (args: { where: { id?: string; userId: string; status?: "UNREAD" }; data: { status: "READ" } }) => {
        let count = 0;
...
test("read transitions update unread and read-all states", async () => {
  const { memoryDb, inboxRows } = createMemoryDb();
  inboxRows.set("d1", { id: "n1", userId: "user-1", type: "SUBMISSION_APPROVED", title: "one", body: "", href: null, dedupeKey: "d1", status: "UNREAD", createdAt: new Date() });
  inboxRows.set("d2", { id: "n2", userId: "user-1", type: "SUBMISSION_REJECTED", title: "two", body: "", href: null, dedupeKey: "d2", status: "UNREAD", createdAt: new Date() });
