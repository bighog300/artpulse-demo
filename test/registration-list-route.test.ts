    ...
    deps: {
      ...
      prisma: {
        $transaction: async <T>(fn: (tx: {
          ...
          registration: {
            findUnique: async () => ({ id: "reg-1", eventId: EVENT_ID, guestEmail: "jane@example.com", confirmationCode: "AP-AAA111", status: "CONFIRMED" }),
            update: async () => ({ id: "reg-1", eventId: EVENT_ID, guestEmail: "jane@example.com", confirmationCode: "AP-AAA111", status: "CANCELLED" }),
          },
        }),
      },
