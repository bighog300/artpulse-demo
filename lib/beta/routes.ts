...
export async function handleAdminPatchRequestStatus(req: NextRequest, params: Promise<{ id: string }>) {
  ...
  await db.betaAccessRequest.update({ where: { id: parsedId.data.id }, data: { status: parsedBody.data.status as BetaAccessRequestStatus } });
