...
const defaultDeps: ImportVenueImageDeps = {
  appDb: db,
...
export async function handleIngestImportVenueImage(
  req: Request,
  params: { runId?: string },
  actorEmail: string,
  deps: Partial<ImportVenueImageDeps> = {},
) {
  const resolved = { ...defaultDeps, ...deps };

  const parsedId = idParamSchema.safeParse({ id: params.runId });
  ...
  const run = await resolved.appDb.ingestRun.findUnique({
    ...
    select: { venueId: true, venue: { select: { websiteUrl: true } } },
  });

  if (!run) return apiError(404, "not_found", "Ingest run not found");
