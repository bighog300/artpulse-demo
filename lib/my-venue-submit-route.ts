...
type SubmitVenueDeps = {
  ...
  upsertSubmission: (input: { venueId: string; userId: string; message?: string }) => Promise<SubmissionRecord>;
  setVenuePublishedDraft: (venueId: string) => Promise<void>;
  enqueueSubmissionNotification?: (input: {
    userId: string;
    email: string;
    submissionId: string;
    status: string;
...
export async function handleVenueSubmit(req: NextRequest, params: Promise<{ id: string }>, deps: SubmitVenueDeps) {
  try {
    ...
    await enforceRateLimit({
      key: principalRateLimitKey(req, `venue-submit:${parsedId.data.id}`, user.id),
      limit: RATE_LIMITS.venueSubmitWrite.limit,
      windowMs: RATE_LIMITS.venueSubmitWrite.windowMs,
    });
