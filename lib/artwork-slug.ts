...
export function slugifyArtworkTitle(title: string) {
  ...
  if (!normalized) return "artwork";
  if (RESERVED_SLUGS.has(normalized)) return `artwork-${normalized}`;
  return normalized;
}

export async function ensureUniqueArtworkSlugWithDeps(
  deps: { findBySlug: (slug: string) => Promise<{ id: string } | null> },
  baseSlug: string,
  excludeId?: string,
) {
  let normalizedBase = normalizeSlugSegment(baseSlug) || "artwork";
  if (RESERVED_SLUGS.has(normalizedBase)) normalizedBase = `artwork-${normalizedBase}`;
