...
export function slugifyEventTitle(title: string) {
  ...
  if (RESERVED_SLUGS.has(normalized)) return `event-${normalized}`;
  return normalized;
}

export async function ensureUniqueEventSlugWithDeps(
  deps: { findBySlug: (slug: string) => Promise<{ id: string } | null> },
  baseSlug: string,
  excludeId?: string,
) {
  let normalizedBase = normalizeSlugSegment(baseSlug) || "event";
  if (RESERVED_SLUGS.has(normalizedBase)) normalizedBase = `event-${normalizedBase}`;
