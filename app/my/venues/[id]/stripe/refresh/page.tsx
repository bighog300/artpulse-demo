...
export default function VenueStripeRefreshPage() {
  ...
  async function refreshLink() {
    setLoading(true);
    try {
      const res = await fetch(`/api/my/venues/${params.id}/stripe/connect`, { method: "POST" });
  ...
  return (
    <main className="mx-auto max-w-2xl space-y-4 p-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <Link className="underline" href={`/my/venues/${params.id}`}>Back to venue dashboard</Link>
      </div>
    </main>
  );
}
