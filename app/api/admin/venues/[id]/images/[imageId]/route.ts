...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string; imageId: string }> }) {
  return withAdminRoute(async ({ actorEmail }) => {
    ...
    return patchAdminEntityImage({ entityType: "venue", entityId: parsedId.data.id, imageId: parsedImageId.data.imageId, actorEmail, req, ...parsedBody.data });
  });
}
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string; imageId: string }> }) {
  return withAdminRoute(async ({ actorEmail }) => {
    ...
    return deleteAdminEntityImage({ entityType: "venue", entityId: parsedId.data.id, imageId: parsedImageId.data.imageId, actorEmail, req });
  });
}
